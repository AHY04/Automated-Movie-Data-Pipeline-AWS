...
영화 정보 API(TMDB)에서 최신 영화 데이터를 가져와 **AWS 클라우드 환경에 자동으로 수집하고 데이터베이스에 저장**하는 자동화 프로젝트입니다.

---

## 📌 1. 프로젝트를 만든 이유
* **목표**: 매일 직접 코드를 실행하지 않아도, 클라우드가 알아서 영화 데이터를 수집하고 저장하는 시스템을 경험해보기 위해 구축했습니다.
* **배경**: 자격증(AWS SAA, SQLD)으로 배운 클라우드와 데이터베이스 이론을 실제 나만의 프로젝트로 적용해보고자 진행했습니다.
...
데이터베이스에 모인 최신 영화 데이터를 바탕으로 실행해본 간단한 SQL 분석 쿼리입니다.
